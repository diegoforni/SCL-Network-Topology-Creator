...
- (e2) **Naming a vector ≠ demonstrating it (mandatory; GENERIC for ANY vulnerability class).** For ANY claim — whether crypto/auth/JWT, injection, access-control, SSRF, deserialization, business-logic, or any other class — a tool / scanner / wordlist / MCP output that NAMES a vulnerability class or vector is a HYPOTHESIS, not evidence. CONFIRMED requires that YOU reproduced the impact yourself this run with a captured artifact: a reproduced exploit primitive, a captured vulnerable-response differential, a cross-role access you reproduced, or a leaked secret you recovered and reused. Naming the class without reproducing the impact => INCONCLUSIVE (or NOT_A_VULN if the class provably does not apply to this target — e.g. an RS↔HS "key confusion" claim against a purely symmetric HS512 deployment, where there is no asymmetric public key to confuse). To CONFIRM you must show the vulnerable signal reproduces AND the clean control (step d) does NOT reproduce it; if you only have a tool's label, you cannot satisfy (1). Record the strongest step you actually took (e.g. "ran hashcat -m 16500 -> exit 255, no GPU", "nuclei tagged idor, but no cross-role 200 captured") in `reason`. A captured vulnerable-response differential (a real 200/302 on a malicious request that the clean role does not get) IS a valid primitive for any class.
...
- (i.1) **JSON SERIALIZATION — NEVER hand-write JSON (mandatory; defeats two real parse failures).** You MUST build EVERY record you write to a `.jsonl` file — both the per-command audit records in (i) and the VERDICT record in (j) — as a Python `dict` and serialize it with `json.dumps(record, ensure_ascii=False)`. NEVER hand-concatenate a JSON string, never interpolate variables into a quoted JSON value, never wrap `$RUN_ID`/`<slug>` in literal double-quotes inside an already-quoted value. Two concrete bugs this prevents: (1) `evidence_file` written as `evidence_file":"'/outputs/"<run_id>"/verifier/\"<slug>\".jsonl"` — run_id/slug wrapped in literal double-quotes INSIDE an already-quoted JSON value → unescaped-quote parse failure; (2) `'Invalid escape'` from Spanish accented text, backslashes, or `\xNN` byte sequences in `output_excerpt`/`reason`. `ensure_ascii=False` keeps UTF-8 (accents, em-dashes, Spanish) intact without emitting `\uXXXX`; if a field contains raw bytes/control chars, decode/replace them before putting them in the dict. The single source of truth is the Python dict; `json.dumps` is the ONLY writer. Canonical pattern (audit record):
  ...
  ```
- (j) **Verdict checkpoint — write the verdict to the audit file FIRST (mandatory; defeats truncation).** The instant you reach your verdict — immediately after the intended-behavior challenge (f) and authorization-model check (g) resolve it, and BEFORE any redundant confirmation re-run and BEFORE your final message — append exactly ONE verdict record to `/outputs/$RUN_ID/verifier/<slug>.jsonl`. Build it as a Python dict and serialize with `json.dumps(record, ensure_ascii=False)` per rule (i.1); the record has exactly these keys: `step`="VERDICT", `claim` (one-line), `defect` (a short stable kebab label for the defect FAMILY — `idor`, `bfla`, `sqli`, `ssrf`, `jwt-key-confusion`, `jwt-key-crack`, `mass-assignment`, `xss`, `cmd-injection`, … — the phase worker's retract-on-contradiction check groups on this label), `verdict` (one of CONFIRMED|NOT_A_VULN|INCONCLUSIVE|NOT_CONFIRMABLE), `ok_to_report` ("YES"|"NO"), `cvss` ("<score> <band>"), `intended_behavior` ("ruled out because ..." | "NOT ruled out"), `reason` (one sentence), and `evidence_file` (the literal string `/outputs/$RUN_ID/verifier/<slug>.jsonl`, expanded by Python from the env — NOT re-quoted). Canonical pattern:
  ```python
  import json, os
  run_id = os.environ["RUN_ID"]
  path = f"/outputs/{run_id}/verifier/{slug}.jsonl"
  os.makedirs(os.path.dirname(path), exist_ok=True)
  record = {"step": "VERDICT", "claim": claim, "defect": defect, "verdict": verdict,
...
**VERDICT RULES:**
- **CONFIRMED** (OK TO REPORT: YES) — only if ALL hold: (1) OBSERVED matches EXPECTED_IF_VULNERABLE AND does not match EXPECTED_IF_CLEAN; (2) the intended-behavior challenge (step f) is ruled out by your own evidence; (3) for authorization claims, the authorization-model check (step g) positively supports the claim — not merely "a sibling endpoint is gated"; (4) every cited artifact was captured by you this run; (5) **you DEMONSTRATED the impact yourself this run** — a captured vulnerable-response differential, a reproduced exploit primitive, a cross-role access you reproduced, or a recovered-and-reused secret (step e2). A tool / scanner / wordlist / MCP output that merely NAMES the class or vector is a hypothesis and cannot satisfy (1) on its own. The vulnerable signal must reproduce and the differential must be real.
