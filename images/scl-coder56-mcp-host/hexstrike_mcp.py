...
def main():
    ...
    try:
        ...
        # --- SCL guardrail integration ------------------------------------------------
        # The executor-side opencode guardrail (guardrail.ts) intercepts every MCP
        # tool call via tool.execute.before and adjudicates it. opencode 1.18.3
        # cannot SKIP a tool from that hook — it always runs the tool afterward —
        # so on a REFUSE the guardrail mutates the call's args IN PLACE to delete
        # the real params and set a `_guardrail_refused` sentinel. We wrap the
        # ToolManager's call_tool (the real tools/call dispatch path — the public
        # FastMCP.call_tool is NOT used by the protocol handler) so that sentinel
        # is honored HERE, before FastMCP parses args / dispatches
        # the tool function: a refused call returns the block message and NEVER
        # reaches the HexStrike backend. This covers the ~32 tools that have no
        # required params (where deleting args alone would still let FastMCP run
        # the function with empty/default args).
        _tm = mcp._tool_manager
        _real_call_tool = _tm.call_tool

        async def _guarded_call_tool(name, arguments, *rest, **kw):
            try:
                if isinstance(arguments, dict) and arguments.get("_guardrail_refused"):
                    reason = arguments.get("_guardrail_refused")
                    logger.info(f"🛑 Guardrail refused MCP tool '{name}': {reason}")
                    # Wrap in {"result": ...}: hexstrike tool outputSchemas require a
                    # top-level `result` (additionalProperties: true), so this validates
                    # cleanly and the agent sees a clear block message, not a schema error.
                    return {
                        "result": {
                            "error": "blocked_by_guardrail",
                            "tool": name,
                            "reason": str(reason),
                            "message": f"MCP tool '{name}' was refused by the guardrail and did not execute.",
                        }
                    }
            except Exception as exc:  # never let the guard break a normal call
                logger.warning(f"guardrail sentinel check failed for {name}: {exc}")
            return await _real_call_tool(name, arguments, *rest, **kw)

        _tm.call_tool = _guarded_call_tool
        # --- end SCL guardrail integration -------------------------------------------
